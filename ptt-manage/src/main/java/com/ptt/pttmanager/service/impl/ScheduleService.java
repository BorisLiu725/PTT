...
//    private StudioMapper studioMapper;
    ...
    private void generateTicket(Schedule schedule){
        ...
        for (int i = 0; i < studio.getSeats().size(); i++) {
            ...
            ticket.setStatus(1);
            //票的二维码
            String ticketQrCode = getTicketQrCode(ticket, studio.getSeats().get(i).getRow(), studio.getSeats().get(i).getColumn());
            ticket.setQrCode(ticketQrCode);
        }
        //批量保存票
     ...
     * 得到票的二维码
     * */
    public String getTicketQrCode(Ticket ticket,Integer rows,Integer colum){
        String prfix = "http://qr.liantu.com/api.php?text=";
        StringBuilder sb = new StringBuilder(prfix);
        Schedule schedule = this.scheduleDao.queryScheduleId(ticket.getScheduleId());
        String movieName = schedule.getMovie().getMovieName();
        String language = schedule.getMovie().getLanguage();
        String movieArea = schedule.getMovie().getMovieArea();
        String studioName = schedule.getStudio().getStudioName();
        Date startTime = schedule.getStartTime();
        Long newPrice = schedule.getNewPrice();

        Map<String,String> msg = new HashMap<String, String>();
        msg.put("电影名：",movieName);
        msg.put("语言：",language);
        msg.put("地区：",movieArea);
        msg.put("演出厅名称：",studioName);
        msg.put("开始时间：",startTime.toString());
        msg.put("价钱：",newPrice.toString());
        msg.put("座位号:",rows+"排 "+colum+"列");
        for(String key : msg.keySet()){
            sb.append(key+":");
            sb.append(msg.get(key));
            sb.append("%0A");
        }
